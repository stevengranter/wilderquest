...
export function createQuestShareController(service: QuestShareService) {
    ...
    async function setObserved(req: Request, res: Response) {
        ...
        const progress = await service.setObservedByToken(token, mappingId, observed);
        res.status(200).json(progress);
    }
    ...
    async function getAggregatedProgress(req: AuthenticatedRequest, res: Response) {
        const questId = Number(req.params.questId);
        const rows = await service.getAggregatedProgressForQuest(questId, req.user?.id);
        res.status(200).json(rows);
    }

    async function getQuestTaxaMappings(req: AuthenticatedRequest, res: Response) {
        const questId = Number(req.params.questId);
    ...
    async function getLeaderboard(req: AuthenticatedRequest, res: Response) {
        const questId = Number(req.params.questId);
        const leaderboard = await service.getLeaderboardForQuest(questId, req.user?.id);
    ...
    async function getDetailedProgress(req: AuthenticatedRequest, res: Response) {
        const questId = Number(req.params.questId);
        const rows = await service.getDetailedProgressForQuest(questId, req.user?.id);
        res.status(200).json(rows);
    }
