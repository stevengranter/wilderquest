					...
					tools: {
						...
						searchWeb: {
							...
							execute: async ({ query, maxResults = 5 }) => {
								try {
									logger.info(`AI requested web search for: "${query}"`);
						...
						searchNews: {
							...
							execute: async ({ query, maxResults = 3 }) => {
								try {
									logger.info(`AI requested news search for: "${query}"`);
							...
							execute: async ({ query, maxResults = 3 }) => {
								try {
									...
									const searchUrl = `https://api.crossref.org/works?query=${encodeURIComponent(
										query,
									)}&rows=${maxResults}&sort=relevance&order=desc`;

									const response = await fetch(searchUrl, {
										headers: {
											'User-Agent':
												'iNaturalist-AI-Assistant/1.0 (mailto:your-email@example.com)',
										},
									});
									...
									logger.error('Error searching scientific papers:', error);
									return {
										success: false,
						...
						setFilters: {
							...
							parameters: z.object({
								filters: z.object({
									...
									ranks: z
										...
										.optional()
										.describe(
											'Array of taxonomic ranks to filter by (e.g., species, genus, family, etc.)',
										),
									hasPhotos: z
										.boolean()
										.nullable()
									...
									dateRange: z
										.object({
											start: z
												.string()
												.optional()
												.describe('Start date inYYYY-MM-DD format'),
											end: z
												.string()
												.optional()
												.describe('End date inYYYY-MM-DD format'),
										})
										.optional(),
							...
							execute: async ({ filters }) => {
								logger.info('AI has requested to set filters:', filters);
								return {
									success: true,
									data: { filters },
						...
						clearFilters: {
							...
							parameters: z.object({}),
							execute: async () => {
								logger.info('AI has requested to clear all filters');
								return {
									success: true,
						...
						suggestFilters: {
							...
							execute: async ({ interest }) => {
								logger.info('AI has requested filter suggestions for:', interest);
						...
						clearSelection: {
							...
							parameters: z.object({}),
							execute: async () => {
								logger.info('AI has requested to clear all selections');
								return {
									success: true,
									data: { selectedIds: [] },
						...
						selectByType: {
							...
							execute: async ({ criteria }) => {
								logger.info('AI has requested to select by criteria:', criteria);
								return {
									success: true,
									data: { criteria },
