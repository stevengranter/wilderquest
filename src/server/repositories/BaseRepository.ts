...
export function createBaseRepository<T>(
    ...
    async function update(
        ...
        const columns = Object.keys(data);
        if (columns.length === 0) throw new Error('No fields provided for update');
    ...
    async function findOne(
        ...
        if (keys.length === 0) throw new Error('findOne called without conditions');
        ...
        const columns = selectColumns.map((col) => `\`${String(col)}\``).join(', ');
    ...
    async function findAll(): Promise<T[]> {
        const [rows] = await dbPool.execute<RowDataPacket[]>(`SELECT * FROM ${tableName}`);
        return rows as T[];
    }
