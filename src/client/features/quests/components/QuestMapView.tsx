...
export const QuestMapView = React.memo(({ 
            ...
            try {
                ...
                for (const batch of batches) {
                    ...
                    try {
                        ...
                        const filtered = response.data.results.filter((r: INatObservation) => r.geojson)
                        allObservations.push(...filtered)
                    } catch (error) {
                        console.error('Failed to fetch observations for batch:', batch, error)
                    }
                }
    ...
    const markers = useMemo(() => {
        return observations.map((obs) => {
            ...
            const speciesName = obs.taxon?.preferred_common_name || 
                               obs.species_guess || 
                               obs.taxon?.name || 
                               'Unknown Species'
            ...
            if (obs.photos?.[0]?.url) {
                ...
                icon = createFallbackMarkerIcon(speciesName, obs.taxon?.rank || 'species')
            }
            
            return {
                position: [obs.geojson.coordinates[1], obs.geojson.coordinates[0]] as [number, number],
                key: obs.id,
                observation: obs,
                icon
            }
                    ...
                    <Marker position={[questData.latitude, questData.longitude]}>
                        <Popup>
                            <div className="text-center">
                                <h3 className="font-semibold">{questData.name}</h3>
                                {questData.location_name && (
                                    <p className="text-sm text-gray-600">{questData.location_name}</p>
                                )}
                            ...
                            <div className="min-w-[200px]">
                                <div className="flex items-center gap-2 mb-2">
                                    <h4 className="font-semibold">
                                        {marker!.observation.taxon?.preferred_common_name || 
                                ...
                                <div className="space-y-1 text-sm text-gray-600">
                                    {marker!.observation.observed_on && (
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span>{new Date(marker!.observation.observed_on).toLocaleDateString()}</span>
                                        </div>
                                    )}
                                    ...
                                    {marker!.observation.place_guess && (
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span>{marker!.observation.place_guess}</span>
                                        </div>
                                    )}
                                    
                                    {marker!.observation.user?.name && (
                                        <p>By: {marker!.observation.user.name}</p>
                                    )}
                                </div>
                            </div>
                        </Popup>
