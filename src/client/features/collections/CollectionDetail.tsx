...
export default function CollectionDetail({
        ...
        const fetchCollection = async () => {
            ...
            try {
                const response = await axios.get(
                    `${API_URL}/${activeCollectionId.toString()}`,
                )
    ...
    if (!collectionData) {
        return (
            <div>
                ...
                <ul>
                    <li>
                        <a href='/collection/123'>Collection 123</a>
                    </li>
                    <li>
                        <a href='/collection/456'>Collection 456</a>
                    </li>
                </ul>
            </div>
        )
    ...
    return (
        <div>
            <h1>
                Collection Detail:{' '}
                {collectionData.name || `ID: ${collectionData.id}`}
            </h1>
            <p>{collectionData.description}</p>
            {/* You can add more details from collectionData here */}
            {propCollectionId && (
                <p> (Rendered via **prop** with ID: {propCollectionId})</p>
            )}
            {!propCollectionId && urlCollectionId && (
                <p>
                    {' '}
                    (Rendered via **URL parameter** with ID: {urlCollectionId})
                </p>
            )}
            <p>
