    ...
    async refreshAccessToken() {
        try {
            ...
            if (response.status !== 200 || !response.data.access_token) {
                ...
                // Example: window.location.href = "/login";
                handleError(`Token refresh failed with status: ${response.status}`); //Crucial to throw an error here
            }
    ...
    verifyToken(token: string | null) {
        if (token) {
            ...
            if (parsedToken.error) {
                console.error("Token decoding error:", parsedToken.error.message);
